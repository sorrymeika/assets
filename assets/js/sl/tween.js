define(['$',"./_linklist","graphics/matrix2d"],function (require) {
    ...
    var TRANSFORM='-webkit-transform',
        ...
        scaleReg=/scale\((\d+\.\d+|\d+)\s*\,\s*(\d+\.\d+|\d+)\)/g,
    ...
    function parallelAnimate(animations) {
        ...
        for(var i=0,n=animations.length,item;i<n;i++) {
            ...
            if(anim.css) {
                ...
                $.each(anim.css,function (key,val) {
                    if(/matrix|translate|skew|rotate|scale|invert/.test(key)) {
                        if(key==='translate') {
                            val=(css[TRANSFORM]||'')+' '+key+'('+val+')';

                        } else {
                            if(!m2d) m2d=new Matrix2D();
                            origTransform=(css[TRANSFORM]||'');
                ...
                el.each(function () {
                    ...
                    console.log(animationStyle,originStyle);
                });
                    ...
                    this.el.each(function () {
                        ...
                        for(var key in style) {
                            ...
                            if(key==TRANSFORM) {
                                ...
                                if(m) {
                                    var i=0;
                                    newStyle[key]=val.replace(matrixReg,function (num) {
